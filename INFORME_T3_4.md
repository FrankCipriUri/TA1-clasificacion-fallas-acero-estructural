...
# Ruta anclada a la ubicacion de este archivo, no a la carpeta desde la que se ejecute
# (funciona igual con el boton "Run" de VS Code o desde la terminal con/sin "cd src").
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
RESULTS_DIR = os.path.join(SCRIPT_DIR, "..", "results")
...
| Fecha | Cambio |
|---|---|
| Entrega inicial | Código, resultados e interpretabilidad documentados con rutas relativas a la carpeta de ejecución (`../data`, `../results`). |
| Actualización 1 | Se incrustó el código fuente completo de ambos scripts directamente en este documento (secciones II.C.1 y II.E.1), y se agregó la salida real de consola de cada ejecución (secciones II.C.2 y II.E.2). |
| Actualización 2 (esta versión) | Se corrigieron las rutas de acceso a `data/` y `results/` en ambos scripts para que estén ancladas a la ubicación del archivo (`os.path.dirname(os.path.abspath(__file__))`) en lugar de la carpeta de trabajo activa. Esto corrige un error (`FileNotFoundError`) que ocurría al ejecutar los scripts con el botón ▶️ ("Run Python File") de VS Code en lugar de la terminal. Verificado ejecutando ambos scripts desde tres contextos distintos (terminal en la raíz, terminal en `src/`, y equivalente al botón ▶️), reproduciendo en los tres casos los mismos resultados reportados en la Sección III. |
